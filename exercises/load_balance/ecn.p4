...
// REMOVE: const bit<19> ECN_THRESHOLD = 10;
...
control MyIngress(inout headers hdr,
    ...
    //ADD: Action to set the ECN threshold metadata
    ...
    //ADD: New table to configure the ECN threshold
    table ecn_config {
        actions = {
            set_ecn_threshold;
            NoAction;
        ...
        size = 1;
        //A default param to make sure it works when theres no action
    }
...
control MyEgress(inout headers hdr,
    ...
    apply {
         ...
         */
         /*REMOVE:(我自己都看不懂自己之前写的啥)
         if(hdr.ipv4.isValid() && (hdr.ipv4.ecn == 1 || hdr.ipv4.ecn == 2)){
            myecn_egress.apply();
         }*/

         if (hdr.ipv4.ecn == 1 || hdr.ipv4.ecn == 2){
            // MODIFIED: Use metadata.ecn_threshold
